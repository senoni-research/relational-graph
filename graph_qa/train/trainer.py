...
def main():
	...
	parser.add_argument("--resume-ckpt", type=str, default=None, help="Path to checkpoint to resume model weights from")
	...
	# Optional: resume from checkpoint before moving to device
	if args.resume_ckpt:
		try:
			ckpt_resume = torch.load(args.resume_ckpt, map_location="cpu")
			state = ckpt_resume.get("model_state", ckpt_resume)
			missing, unexpected = model.load_state_dict(state, strict=False)
			print(f"  → Resumed weights from {args.resume_ckpt} (missing={len(missing)}, unexpected={len(unexpected)})")
		except Exception as e:
			print(f"[warn] failed to resume from {args.resume_ckpt}: {e}")
